...
This repo packages [Ollama](https://github.com/ollama/ollama) for StartOS.
...
The `Makefile` produces two variants — `generic` (CPU + NVIDIA) and `rocm` (AMD GPU) — for each supported architecture.

## Updating the upstream version

Ollama is pulled in as the official upstream image, `ollama/ollama`, in two flavors. Both tags are pinned in `startos/manifest/index.ts` and must be bumped together so the generic and ROCm variants stay aligned.

1. Bump both `dockerTag` values in `startos/manifest/index.ts`:
   - `imageConfigs.generic.source.dockerTag` → `ollama/ollama:<new version>`
   - `imageConfigs.rocm.source.dockerTag` → `ollama/ollama:<new version>-rocm`
